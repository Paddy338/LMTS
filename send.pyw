    ...
    def load_ip_list(self):
        try:
            with open(self.file, 'r', encoding = 'utf-8') as f:
                ...
                if not isinstance(self.ip_data, list): # 判断ip_list是不是列表类型
                    self.ip_data = []
        except (json.JSONDecodeError, FileNotFoundError):
            self.ip_data = [] # 暂时不处理解码错误
...
class UDPNetwork:
    ...
    def send_socket(self, dest_ip: str, msg: str):
        ...
        try:
            (status, addr)=self.s.recvfrom(1024)
...
class MainWindow:
    ...
    def render_ui(self):
        # 工具栏
        toolbar = ttk.Frame(self.root, bootstyle = "light")
        toolbar.pack(side = "top", fill = "x")
        try:
            self.about_icon = ttk.PhotoImage(file = "icons/about.png")
            self.about_btn = ttk.Button(toolbar, image = self.about_icon, command = show_about, bootstyle = "link")
        except:
            self.about_btn = ttk.Button(toolbar, text = "关于", command = show_about, bootstyle = "link")
        self.about_btn.pack(side = "right", padx = 5)

        # 输入IP地址
        self.ipaddr_form = ttk.Labelframe(self.root, padding = 10, text = "目标 IP 地址")
        self.ipaddr_form.pack(side = LEFT, fill = Y, padx = 10, pady = 10)
        self.ip_entry_frame = ttk.Frame(self.ipaddr_form)
        self.ip_entry_frame.pack(pady = 5)
        self.ip_combobox = ttk.Combobox(self.ip_entry_frame, width = 20, cursor = "ibeam")
        self.ip_combobox["values"] = self.iplistmgr.get_all_ip()
        self.ip_combobox.pack(side = LEFT, padx = 5)
        self.add_button = ttk.Button(self.ip_entry_frame,
                                     text = "添加",
                                     command = self.add_new_ip,
                                     bootstyle = "secondary")
        self.add_button.pack(side = RIGHT)

        self.input_hint_str = "在输入框中输入 IP 地址。\n点击“添加”将其固定至下拉选项。"
        self.input_hint = ttk.Label(self.ipaddr_form, text = self.input_hint_str)
        self.input_hint.pack(padx = 5, pady = 5)

        default_font = tkfont.Font(family = "Courier",
                            size = 12)
        self.msg_input = ttk.Text(window, width = 80, height = 25, font = default_font)
        self.msg_input.pack(padx = 5, pady = 5, fill = BOTH, expand = True)

        self.urgent_frm = ttk.Frame(window)
        ...
        self.send_btn = ttk.Button(window,
                            text = "发送",
    ...
    def do_send(self):
        ...
        if result == "received":
            ...
            tkmsgbox.showerror(title = "错误",
                                message = f'''网络错误：连接已重置。\n请检查 IP 地址是否正确、对端电脑是否已开启接收端、网络是否畅通。''')
        
if __name__=="__main__":
    iplist = IPListManager("ips.json")
    network = UDPNetwork()
